...
def split_inputs_cp(x: Tensor, seq_dim: int, cp_group: ProcessGroup) -> Tensor:
    ...
    x = x.view(
        *x.shape[:seq_dim], cp_size, x.shape[seq_dim] // cp_size, *x.shape[(seq_dim + 1) :]
    )  # [...,cp_size,S/cp_size,...]
    seq_idx = torch.tensor([cp_group.rank()], device=x.device)  # [1]
    x = x.index_select(seq_dim, seq_idx)  # [...,1,S/cp_size,...]
    # Note that the new sequence length is the original sequence length / cp_size
    x = x.view(*x.shape[:seq_dim], -1, *x.shape[(seq_dim + 2) :])  # [...,S/cp_size,...]
    return x
...
def cat_outputs_cp(x: Tensor, seq_dim: int, cp_group: ProcessGroup) -> Tensor:
    ...
    return torch.cat(gathered_tensors, dim=seq_dim)  # [...,S*cp_size,...]
...
def robust_broadcast(tensor: torch.Tensor, src: int, pg: ProcessGroup, is_check_shape: bool = False) -> torch.Tensor:
    ...
    if distributed.get_rank() == src:
        shape = torch.tensor(tensor.shape, dtype=torch.long).cuda()  # [ndim]
    else:
        shape = torch.empty(tensor.dim(), dtype=torch.long).cuda()  # [ndim]
    if is_check_shape:
        _verify_param_shape_across_processes(pg, [shape])
    torch.distributed.broadcast(shape, src, group=pg)  # [ndim]
