...
# Map string dtype names (as stored in TrainConfig.param_dtype) to torch dtypes.
...
class HFExportCallback(Callback):
    ...
    Args:
        dtype: Export weight dtype (e.g. ``"bfloat16"``).  Use
            ``"${model.config.train.param_dtype}"`` in the Hydra callback config to inherit from
