...
class VLMModel(ImaginaireModel):
    ...
    def _init_vlm(self, policy, checkpoint, train) -> None:
        ...
        _tp = policy.parallelism.tp_size
        _ep = policy.parallelism.ep_size
        _pp = policy.parallelism.pp_size
        if _tp != 1 or _ep != 1 or _pp != 1:
            raise NotImplementedError(
                "VLMModel: tp/ep/pp parallelism is not supported in the unified ParallelDims "
                f"design (got tp={_tp}, ep={_ep}, pp={_pp}). Set policy.parallelism.tp_size=1, "
                f"ep_size=1, pp_size=1, and use dp_shard_size + dp_replicate_size for FSDP/HSDP."
            )
        _dp_shard = policy.parallelism.dp_shard_size
        _dp_replicate = policy.parallelism.dp_replicate_size
        ...
        parallel_dims = ParallelDims(
            ...
            cp=getattr(policy.parallelism, "cp_size", 1),
        ...
        # ── d. Apply FSDP2 ──
        if torch.distributed.is_initialized():
            parallelize(hf_model, parallel_dims, train)
        ...
        if getattr(train, "fsdp_offload", False):
        ...
        if policy.model_gradient_checkpointing:
