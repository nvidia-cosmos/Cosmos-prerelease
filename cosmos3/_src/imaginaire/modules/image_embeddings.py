...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        x = x.reshape(x.shape[0], x.shape[1], x.shape[2] * x.shape[3]).permute(2, 0, 1)  # [HW,B,C]
        x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # [HW+1,B,C]
        x = x + self.positional_embedding[:, None, :].to(x.dtype)  # [HW+1,B,C]
        x, _ = F.multi_head_attention_forward(  # x: [HW+1,B,output_dim]
            query=x,
            key=x,
            value=x,
            ...
            in_proj_weight=None,
            in_proj_bias=torch.cat([self.q_proj.bias, self.k_proj.bias, self.v_proj.bias]),  # [3*embed_dim]
            bias_k=None,
            bias_v=None,
            add_zero_attn=False,
            dropout_p=0,
...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.type(self.conv1.weight.dtype)  # [B,3,H,W]
        x = stem(x)  # [B,width,H/4,W/4]
        x = self.layer1(x)  # [B,width*4,H/4,W/4]
        x = self.layer2(x)  # [B,width*8,H/8,W/8]
        x = self.layer3(x)  # [B,width*16,H/16,W/16]
        x = self.layer4(x)  # [B,width*32,H/32,W/32]
        x = self.attnpool(x)  # [B,output_dim]
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x + self.positional_embedding.to(x.dtype)  # [B,grid**2+1,width]
        x = self.ln_pre(x)  # [B,grid**2+1,width]

        x = x.permute(1, 0, 2)  # [grid**2+1,B,width]
        x = self.transformer(x)  # [grid**2+1,B,width]
        x = x.permute(1, 0, 2)  # [B,grid**2+1,width]

        x_pre_pooling = x  # [B,grid**2+1,width]
...
class CLIP(nn.Module):
    ...
    def build_attention_mask(self):
        ...
        mask = torch.empty(self.context_length, self.context_length)  # [context_length,context_length]
    ...
    def encode_text(self, text):
        ...
        """
        x = self.token_embedding(text).type(self.dtype)  # [B,n_ctx,transformer_width]

        x = x + self.positional_embedding.type(self.dtype)  # [B,n_ctx,transformer_width]
        ...
        x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection  # [B,embed_dim]
    ...
    def forward(self, image, text):
        ...
        """
        image_features, _ = self.encode_image(image)  # [B,embed_dim]
        text_features = self.encode_text(text)  # [B,embed_dim]

        # normalized features
        image_features = image_features / image_features.norm(dim=1, keepdim=True)  # [B,embed_dim]
        text_features = text_features / text_features.norm(dim=1, keepdim=True)  # [B,embed_dim]
