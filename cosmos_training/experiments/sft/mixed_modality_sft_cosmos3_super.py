...
"""LoRA mixed-modality SFT on Qwen3-VL-32B MoT — "cosmos3 super".

T2V / I2V / V2V at 70/20/10 ratio. Only the LoRA adapters on the gen MoE
attention projections are trained — the rest of the backbone stays frozen.
...
Usage::
    ...
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. torchrun --nproc_per_node=4 \\
        --master_port=12341 -m scripts.train \\
        --config=configs/base/config.py -- \\
        experiment=mixed_modality_sft_cosmos3_super \\
...
mixed_modality_sft_cosmos3_super = LazyDict(
    dict(
        defaults=[
        ...
        job=dict(
            project="cosmos3",
            group="sft",
            name="mixed_modality_sft_cosmos3_super",
            wandb_mode="disabled",
        ),
        data_setting=dict(
            ...
            config=dict(
                ...
                vlm_config=dict(
                    ...
                    pretrained_weights=dict(
                        enabled=False,
                        backbone_path="s3://bucket/cosmos3/pretrained/huggingface/Qwen/Qwen3-VL-32B-Instruct/",
...
for _item in [mixed_modality_sft_cosmos3_super]:
