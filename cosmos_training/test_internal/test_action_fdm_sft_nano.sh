...
# Internal smoke for action_fdm_sft_nano (Bridge forward-dynamics SFT, 4-GPU FSDP,
# 20 iters) inside the released cosmos_training tree.
#
# Counterpart of imaginaire4/test_internal/test_action_fdm_sft_nano.sh but:
#   - WORKDIR is the released tree
#   - --config is configs/base/config.py
#   - DATASET_PATH points at the local Bridge LeRobot v3 snapshot
#
# Usage:
#   bash test_internal/test_action_fdm_sft_nano.sh

set -uo pipefail

WORKDIR="/home/pzeren/lustre/Cosmos-prerelease/cosmos_training"
# BridgeOrigLeRobotDataset uses a single ``root`` path (taken from $DATASET_PATH
# by the experiment). The HF snapshot dir contains data/, meta/, videos/, etc.
DATASET_PATH="${DATASET_PATH:-/nfs/sw/sw_aidot/users/pzeren/.cache/huggingface/hub/datasets--nvidia--bridge_lerobot_v3/snapshots/b887e193b141f2fe5b6e3d567577aa51c475693b}"
...
#   CHECKPOINT_PATH=/your/path bash test_internal/test_action_fdm_sft_nano.sh
...
LOG_FILE="$LOG_DIR/action_fdm_sft_nano_debug.log"
...
LOGURU_LEVEL=DEBUG IMAGINAIRE_OUTPUT_ROOT="$OUTPUT_ROOT" PYTHONPATH=. \
    ...
    job.group=debug \
    job.name=action_fdm_sft_nano_debug \
